...
uses
  ...
  Codecs, fuzzySearch, strutils, math, ProgressHandling, remote_simulator,
  remote_simlator_form;

const
  VERSION = 'Version 0.0.20 - Developement, fpc 3.0.4, Lazarus 1.8.4'; //< Version information.
...
type
  ...
  TMainForm = class(TForm)
    ...
    SimulationModeCheckBox: TCheckBox;
    Label2: TLabel;
    ...
    procedure SimulationModeCheckBoxClick(Sender : TObject);
  ...
  private
    ...
    function transmittMessage(Message : string) : string;
...
function TMainForm.transmittMessage(Message : string) : string;
begin
  Result := 'N/A';
  if SimulationModeCheckBox.State = cbChecked then
  begin
    Remote_station_simulator_Form.Simulator_Receive_Memo.Append(Message);
    Result := 'Simulated sent';
  end
  else
  begin
    Result := HTTPClient.PutMessage(Message);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        LastMessage := transmittMessage(MyMessage);
        {$ifdef Debug}
        ...
        begin
          ...
          begin
            ...
            MyMessage := LF + RemoteCallSign + ' de ' + MyCallSign + LF
              ...
              + ' eee eee eee eee';
            LastMessage := transmittMessage(MyMessage + LF);
            {$ifdef Debug}
              MainMemo.Append(MyMessage);
            {$endif}
          end;
...
//            RXWidgetTimer.Enabled := true;
          ...
          LastMessage := transmittMessage(EncStrings.Strings[teller]+LF);
          {$ifdef Debug}
...
procedure TMainForm.SimulationModeCheckBoxClick(Sender : TObject);
begin
  if SimulationModeCheckBox.State = cbUnchecked then
  begin
    FreeAndNil(Remote_station_simulator_Form);
  end
  else
  begin
    Remote_station_simulator_Form := TRemote_station_simulator_Form.Create(nil);
    Remote_station_simulator_Form.Show;
  end;
end;
